...
class RefundController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->has('rows')) {
            $filteredRows = collect($request->input('rows'))->filter(function ($row) {
                return !empty($row['id']);
            })->values()->toArray();
        ...
        $request->validate([
            ...
            'rows.required' => 'Please add at least one valid item to process the refund.',
            'rows.min' => 'Please add at least one valid item to process the refund.'
        ]);
        ...
        try {
            ...
            // Deduct Refund Amount from Active Wallet Balance
