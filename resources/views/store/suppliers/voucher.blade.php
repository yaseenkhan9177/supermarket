...
<head>
    ...
    <title>Supplier Payment Voucher - {{ $voucher->voucher_number }}</title>
...
<body class="bg-slate-100 dark:bg-slate-900 min-h-screen font-sans text-slate-800 dark:text-slate-200 antialiased p-4 md:p-8 flex flex-col items-center">
    ...
    <div class="no-print w-full max-w-2xl flex items-center justify-between mb-6">
        ...
        <button onclick="window.print()" class="inline-flex items-center gap-2 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 px-5 py-2.5 rounded-xl shadow-md hover:shadow-lg transition">
            <i class="fas fa-print"></i> Print Voucher
        </button>
    </div>

    {{-- Printable Voucher Container --}}
    <div class="voucher-card relative bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-xl p-8 md:p-10 w-full max-w-2xl overflow-hidden">
        ...
        <div class="flex flex-col md:flex-row items-center justify-between border-b border-slate-200 dark:border-slate-700 pb-6 mb-8 gap-4 text-center md:text-left">
            ...
            <div class="text-center md:text-right">
                <span class="inline-block px-3.5 py-1 text-xs font-black uppercase tracking-wider rounded-full bg-blue-100 text-blue-800 dark:bg-blue-950/60 dark:text-blue-300 mb-1">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">
                    Date: {{ \Carbon\Carbon::parse($voucher->created_at)->format('d M Y, h:i A') }}
                </p>
            </div>
        </div>
        ...
        <div class="bg-slate-50 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-700/60 rounded-2xl p-5 mb-8 flex flex-col md:flex-row justify-between gap-4">
            <div>
                <span class="text-xs uppercase font-bold text-slate-400 dark:text-slate-500 tracking-wider">Paid To Supplier</span>
                <h3 class="text-xl font-black text-slate-900 dark:text-white mt-0.5">
                    {{ $voucher->supplier->name ?? 'Supplier' }}
                </h3>
                @if($voucher->supplier && $voucher->supplier->company_name)
                    <p class="text-xs font-semibold text-slate-600 dark:text-slate-400">
                        {{ $voucher->supplier->company_name }}
                    </p>
                @endif
                @if($voucher->supplier && $voucher->supplier->phone)
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                        <i class="fas fa-phone text-slate-400 mr-1"></i> {{ $voucher->supplier->phone }}
                    </p>
                @endif
            </div>
            ...
            <div class="text-left md:text-right">
                ...
                <p class="text-base font-bold text-slate-800 dark:text-slate-200 capitalize mt-0.5">
                    <i class="fas fa-money-bill-transfer text-blue-600 mr-1"></i> {{ str_replace('_', ' ', $voucher->payment_method) }}
                </p>
            </div>
        </div>

        {{-- Main Amount Card --}}
        <div class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/20 border border-blue-200/80 dark:border-blue-800/50 rounded-2xl p-6 mb-8 text-center">
            <span class="text-xs uppercase font-black tracking-widest text-blue-600 dark:text-blue-400">
                Amount Paid
            </span>
            <div class="text-4xl md:text-5xl font-black text-blue-700 dark:text-blue-300 mt-1">
        ...
        <div class="space-y-4 mb-8 text-sm border-t border-b border-slate-100 dark:border-slate-700/60 py-6">
            <div class="flex justify-between items-center">
                <span class="text-slate-500 dark:text-slate-400 font-medium">Remaining Payable Balance:</span>
                <span class="font-mono font-bold text-slate-900 dark:text-white text-base">
                    Rs. {{ number_format($voucher->remaining_balance, 2) }}
                </span>
            </div>
            
            <div class="flex justify-between items-center">
                <span class="text-slate-500 dark:text-slate-400 font-medium">Paid By:</span>
                <span class="font-semibold text-slate-800 dark:text-slate-200">
                    <i class="fas fa-user-circle text-slate-400 mr-1"></i>
                    {{ $voucher->paidBy->name ?? 'Store Admin' }}
                </span>
            </div>

            @if($voucher->ledgerEntry && $voucher->ledgerEntry->note)
        ...
        <div class="pt-6 flex flex-col md:flex-row justify-between items-center gap-6 text-xs text-slate-400 dark:text-slate-500">
            <div>
                <p class="font-medium">Computer generated voucher — valid without stamp.</p>
                <p class="text-[10px] text-slate-400">Payment Document Copy</p>
            </div>
