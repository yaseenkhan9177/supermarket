...
@section('navbar_subtitle', 'Cash Refund')

@section('content')
<div x-data @customer-selected.window="onCustomerSelected($event.detail)">

    <form action="{{ route('refunds.store') }}" method="POST" id="refundForm" class="flex-grow flex flex-col space-y-4">
        @csrf

        <!-- Top Form Area -->
        <div class="bg-white border border-gray-300 p-4 rounded-xl shadow-sm shrink-0 text-gray-800">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                <div class="col-span-1 md:col-span-6 grid grid-cols-12 gap-2 items-center">
                    ...
                    <div class="col-span-9">
                        <textarea id="customer_address" rows="2" class="w-full border border-gray-300 text-sm p-2 rounded-lg bg-gray-50 text-gray-600 outline-none" readonly></textarea>
                    </div>

                    <label class="col-span-3 text-xs font-bold text-gray-700 text-right">Phone/Fax</label>
                    <div class="col-span-9 flex gap-2">
                        <input type="text" id="customer_phone" class="w-full border border-gray-300 text-sm p-2 rounded-lg bg-gray-50 text-gray-600 outline-none" readonly>
                    </div>
                </div>
                ...
                <div class="col-span-1 md:col-span-6 grid grid-cols-12 gap-2 items-center md:border-l border-gray-200 md:pl-4 pt-4 md:pt-0 border-t md:border-t-0">
                    ...
                    <div class="col-span-4">
                        <input type="text" name="credit_no" value="{{ $creditNo }}" class="w-full border border-gray-300 text-sm p-2 rounded-lg bg-gray-50 font-mono text-gray-700" readonly>
                    </div>

                    <label class="col-span-1 text-xs font-bold text-gray-700 text-right">Date</label>
                    <div class="col-span-4">
                        <input type="date" name="refund_date" value="{{ date('Y-m-d') }}" class="w-full border border-gray-300 text-sm p-2 rounded-lg">
                    </div>

                    <label class="col-span-3 text-xs font-bold text-gray-700 text-right">Paid From</label>
                    <div class="col-span-9">
                        <select name="paid_from_account_id" class="w-full border border-gray-300 text-sm p-2 rounded-lg">
                    ...
                    <div class="col-span-9">
                        <select name="sales_rep_id" class="w-full border border-gray-300 text-sm p-2 rounded-lg">
                    ...
                    <div class="col-span-9">
                        <input type="text" name="memo" class="w-full border border-gray-300 text-sm p-2 rounded-lg">
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- Grid/Table Area -->
        <div class="flex-grow min-h-[250px] bg-white border border-gray-300 shadow-inner overflow-auto relative rounded-xl text-gray-800">
        ...
        <!-- Footer Controls -->
        <div class="bg-gray-200 border-t border-gray-300 p-3 rounded-xl shrink-0">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">

                <div class="flex space-x-2 w-full md:w-auto justify-center md:justify-start">
                    <button type="button" onclick="addNewRow()" class="flex flex-col items-center px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-blue-50 shadow-sm transition active:scale-95 text-xs font-bold">
                        <span class="text-green-600 text-base leading-none">+</span>
                        <span class="text-[10px] font-bold text-gray-600">Add Row</span>
                    </button>
                    <button type="submit" class="flex flex-col items-center px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shadow-sm transition active:scale-95 text-xs font-bold">
                        <i class="fas fa-save mb-0.5"></i>
                        <span>Save Refund</span>
                    </button>
                    <a href="{{ route('refunds.index') }}" class="flex flex-col items-center px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-red-50 shadow-sm transition no-underline active:scale-95 text-xs font-bold">
                        <i class="fas fa-times text-red-500 mb-0.5"></i>
                        <span class="text-gray-600">Close</span>
                    </a>
                </div>

                <div class="flex items-center justify-between w-full md:w-auto space-x-4 bg-white px-4 py-2 rounded-lg border border-gray-300 shadow-sm">
...
<script>
    ...
    function onCustomerSelected(detail) {
        ...
        if (detail.customer) {
            if (addressEl) addressEl.value = detail.customer.address || '';
            if (phoneEl) phoneEl.value = detail.customer.phone || '';
        } else if (detail.id) {
            fetch(`/api/customers/${detail.id}`)
                .then(res => res.json())
                .then(cust => {
