...
<body>
    ...
    @endphp
    <div class="header">
        <div class="store-name" style="text-transform: uppercase;">{{ $storeName }}</div>
        ...
        @endif
        <div style="font-weight: bold; margin-top: 3px;">Retail Invoice</div>
    </div>
    ...
    <div class="totals">
        <div>Subtotal: {{ number_format($sale->subtotal ?? $sale->grand_total, 2) }}</div>
        @if(($sale->return_adjustment ?? 0) > 0)
        <div>Return Adj: -{{ number_format($sale->return_adjustment, 2) }}</div>
        @endif
