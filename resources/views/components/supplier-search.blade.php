...
<div x-data="{
    ...
    onInput() {
        ...
        this.debounceTimer = setTimeout(() => {
            fetch('/suppliers/search?q=' + encodeURIComponent(this.query.trim()))
                .then(res => res.json())
                .then(data => {
                    this.results = data || [];
                    this.loading = false;
    ...
    getGroupedResults() {
        ...
        const groups = {};
        this.results.forEach(sup => {
            const letter = sup.name ? sup.name.trim().charAt(0).toUpperCase() : '#';
            if (!groups[letter]) groups[letter] = [];
            groups[letter].push(sup);
        });
    ...
    triggerAddNew() {
        this.isOpen = false;
        this.$dispatch('open-add-supplier-modal');
    },
    ...
    <div class="relative flex items-center">
        <input 
            ...
            x-model="query" 
            @input="onInput()" 
            @focus="if (query.trim().length >= 1 || {{ $addNew ? 'true' : 'false' }}) isOpen = true"
    ...
    >
        ...
        <template x-for="group in getGroupedResults()" :key="group.letter">
            <div>
                <div class="px-3 py-1 bg-gray-900/90 text-[10px] font-black text-purple-400 uppercase tracking-wider sticky top-0 border-y border-gray-700/60 backdrop-blur-sm flex justify-between items-center">
                ...
                <template x-for="sup in group.items" :key="sup.id">
                    <button 
                        type="button" 
                    ...
                    >
                        <div class="flex justify-between items-center w-full">
                            <div class="flex items-center gap-1.5 truncate">
                                <span class="font-bold text-gray-100 group-hover:text-purple-300" x-text="sup.name"></span>
                                <span x-show="sup.code" class="text-[10px] bg-gray-700 text-gray-300 px-1 py-0.5 rounded font-mono" x-text="sup.code"></span>
                            </div>
                            <span x-show="sup.phone" class="text-[11px] text-gray-400 font-mono shrink-0 ml-2" x-text="sup.phone"></span>
                        </div>
                        <div x-show="sup.company_name || sup.current_balance" class="flex justify-between items-center text-[10px] text-gray-400 mt-0.5">
                            <span x-text="sup.company_name ? sup.company_name : ''" class="truncate max-w-[200px]"></span>
                            <span x-show="sup.current_balance != 0" :class="sup.current_balance > 0 ? 'text-red-400 font-semibold' : 'text-green-400 font-semibold'" x-text="(sup.current_balance > 0 ? 'Payable: Rs.' : 'Credit: Rs.') + Math.abs(parseFloat(sup.current_balance)).toFixed(2)"></span>
                        </div>
                    </button>
                </template>
            </div>
        ...
        <div x-show="!loading && results.length === 0 && query.trim().length >= 1" class="p-3 text-center text-xs text-gray-400">
            No suppliers found for "<span x-text="query"></span>".
        </div>
    </div>
</div>
